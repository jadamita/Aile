...
namespace Aile.Core.Headers
{
    public class DOSHeader
    {
        ...
        private List<IHeaderField> dosHeaderFields;

        public DOSHeader(byte[] headerBytes)
        {
            dosHeaderFields = new List<IHeaderField>();
        ...
        {
            dosHeaderFields.Add(new ShortHeaderField("Magic Number", _headerBytes));
        ...
        {
            foreach (IHeaderField headerField in this.dosHeaderFields)
            {
                headerField.Parse();
            }
        }
